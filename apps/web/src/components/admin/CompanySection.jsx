                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: "0.45rem" }}>
                    {value.map((tag) => (
                        <span key={tag} style={{ display: "inline-flex", alignItems: "center", gap: "0.35rem", padding: "0.26rem 0.55rem", borderRadius: "999px", background: "rgba(255,255,255,0.06)", border: "1px solid var(--border, #333)", fontSize: "0.78rem" }}>
                            {tag}
                            <button type="button" onClick={() => removeTag(tag)} style={{ background: "none", border: "none", color: "inherit", cursor: "pointer", padding: 0, lineHeight: 1 }}>x</button>
                        </span>
                    ))}
                </div>
            ) : null}
            <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: "0.5rem" }}>
                <input className="settings-input" value={input} placeholder={placeholder || "Agregar..."} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); addTag(); } }} />
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(320px, 1fr))", gap: "1rem" }}>
                        <Card title="Empresa" copy="Datos fijos de negocio que usa la IA al hablar de tu clinica.">
                            <div style={gridTwo}>
                                ...
                                <div style={{ gridColumn: "1 / -1" }}><Field label="Servicios que NO ofrecemos" hint="el bot los descarta"><Tags value={Array.isArray(company.restrictions) ? company.restrictions : []} onChange={(next) => setCompany((prev) => ({ ...prev, restrictions: next }))} placeholder="Ej: manos, manicure..." /></Field></div>
                            </div>
                        </Card>
                        <Card title="Personalidad del Bot" copy="Define la voz de Podito cuando la IA responde al cliente.">
                            <div style={gridTwo}>
                                ...
                                <div style={{ gridColumn: "1 / -1" }}><Field label="Emojis frecuentes"><Tags value={Array.isArray(botIdentity.emojis) ? botIdentity.emojis : []} onChange={(next) => setBotIdentity((prev) => ({ ...prev, emojis: next }))} placeholder="Ej: saludo, apoyo, cierre" /></Field></div>
                            </div>
                        </Card>
                    </div>
                    <div style={{ display: "flex", justifyContent: "flex-end" }}><button type="submit" className="btn btn-primary" disabled={saving}>{saving ? "Guardando..." : "Guardar identidad"}</button></div>
                </form>
            ) : null}
            {tab === "branches" ? (
                <div style={{ display: "grid", gap: "1rem" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", gap: "1rem", alignItems: "center", flexWrap: "wrap" }}>
                        ...
                        <button type="button" className="btn btn-primary btn-sm" onClick={() => editBranch(null)}>+ Agregar sucursal</button>
                    </div>
            ...
            {tab === "services" ? (
                <div style={{ display: "grid", gap: "1rem" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", gap: "1rem", alignItems: "center", flexWrap: "wrap" }}>
                        ...
                        <button type="button" className="btn btn-primary btn-sm" onClick={() => editService(null)}>+ Agregar servicio</button>
                    </div>
